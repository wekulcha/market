  ...
  return (
    <MiniAppShell>
      ...
      <main className="mt-4 space-y-4 pb-20">
        <section className="rounded-2xl border border-emerald-100 bg-white p-4 text-slate-950 shadow-sm">
          <div className="flex items-end justify-between gap-3">
            <div>
              <div className="text-xs font-semibold text-emerald-700">Средняя оценка</div>
              <div className="mt-1 flex items-end gap-2">
                <span className="text-4xl font-bold leading-none">
                  {isLoading ? '—' : formatAverageRating(summary?.average_rating ?? null)}
                </span>
                <span className="pb-1 text-sm text-amber-500">
                  {reviewStars(summary?.average_rating ?? 0)}
                </span>
              </div>
            </div>
            <div className="rounded-2xl bg-emerald-50 px-3 py-2 text-right">
              <div className="text-xl font-bold leading-none">{summary?.reviews_count ?? 0}</div>
          ...
          {reviews.length > 0 && (
            <div className="mt-4 space-y-2">
              {ratingDistribution.map((item) => (
                <div key={item.rating} className="grid grid-cols-[28px_1fr_24px] items-center gap-2 text-xs">
        ...
        {error && (
          <section className="rounded-2xl border border-amber-100 bg-amber-50 p-3 text-xs text-amber-800">
        ...
        {!isLoading && !error && reviews.length === 0 && (
          <section className="rounded-2xl bg-white p-4 text-center text-sm text-slate-500 shadow-sm">
            Отзывов пока нет.
          </section>
        )}
